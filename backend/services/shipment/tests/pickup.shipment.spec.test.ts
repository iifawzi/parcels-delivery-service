...
describe("[Shipments APIs] | pickupShipment API", () => {
    ...
    beforeAll(async () => {
        if (process.env.NODE_ENV === 'integration') {
            ...
            await ShipmentModel.insertMany([
                ...
                {
                    _id: new mongoose.Types.ObjectId("63a271ebbe91afafb4d48c66"),
                    customer: new mongoose.Types.ObjectId("63a22b00a704bee4b0254f4d"),
                    pickUpAddress: "Egypt",
                    pickOfAddress: "Germany",
    ...
    describe("[Given] Someone is trying to pickup a shipment", () => {
        ...
        describe('[When] Biker is trying to pickup a shipment', () => {
            ...
            describe('[When] Biker is trying to pickup shipment with missing information', () => {
            ...
            describe('[Given] Biker is trying to pickup with correct information', () => {
                ...
                describe('[When] a biker is trying to pickup another biker\'s shipment', () => {
                    it("[Then] Should respond with 409", async () => {
                        let res = await request(server)
                            .patch("/api/shipment/pickup")
                            .auth(bikerToken, { type: 'bearer' })
                ...
                describe('[When] Shipment is not in the correct state to be picked', () => {
                ...
                describe('[When] Shipment can be picked', () => {
                    it("[Then] Should respond with 200", async () => {
                        let res = await request(server)
                            .patch("/api/shipment/pickup")
                            .auth(bikerToken, { type: 'bearer' })
                            .send({ shipmentId: "63a271ebbe91afafb4d48c66" });
